...
if [ "$mode" = "marvin" ]; then
        ...
        singularity run --nv /opt/images/cellprofiler/cellprofiler-4_2_6.sif -c -r -p parasitecolocalizationadj.cppipe -i data -o results --images-per-batch=12
elif [ "$mode" = "local" ]; then
        echo "[==> Processing plate on local machine"
        cellprofiler -c -r -p parasitecolocalizationadj.cppipe -i data -o results --images-per-batch=12 --plugins-directory=${plugins_directory}
else
